...
def patched_findScript(gesture):
	...
	if mainKeyName and mainKeyName.endswith("Arrow"):
		...
		if (
			...
			and gesture.normalizedIdentifiers[0].split(":")[1]
			in [
				"alt+downarrow+shift",
				"alt+leftarrow+shift",
...
class FullscreenView(View):
	...
	def isAtEdge(self, orientation):
		...
		isAtBottomEdge = orientation == "down" and self.top + 1 >= self.screen.height * (
			1 - 1 / self.zoomLevel
		)
		isAtRightEdge = orientation == "right" and self.left + 1 >= self.screen.width * (
			1 - 1 / self.zoomLevel
		)
		return self.zoomLevel == 1 or isAtTopEdge or isAtLeftEdge or isAtBottomEdge or isAtRightEdge
...
class LensView(View):
	...
	def isAtEdge(self, orientation):
		...
		return isAtTopEdge or isAtLeftEdge or isAtBottomEdge or isAtRightEdge
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def _sendWillDisablesNumlock(self, gesture):
		...
		if buildVersion.version_year < 2025 and "numlock" in gesture.normalizedIdentifiers[0].split(":")[1]:
			return True
		return False
	...
	def updateKeepMagWindowOnTop(self, keepOnTop):
		...
		try:
			winUser2.setWindowPos(
				...
				cx=0,
				cy=0,
				uFlags=winUser2.SWP_NOSIZE
				| winUser2.SWP_NOMOVE
				| winUser2.SWP_NOACTIVATE
	...
	def script_toggleColorFiltering(self, gesture):
		if not (config.conf["winMag"]["reportColorFilter"] and getColorFilteringKeyValue("HotkeyEnabled")):
			gesture.send()
			return
		val = _WaitForValueChangeForAction(
			gesture,
